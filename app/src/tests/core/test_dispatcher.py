...
from katomato.core.dispatcher import command_dispatcher, controller_dispatcher
from katomato.core.sensor_data import SensorData
from katomato.core.registry import CONTROLLER_REGISTRY
...
async def test_command_dispatcher_switches_phase(command_queue, fake_arduino_protocol, fake_shutdown_handler):
    ...
    with patch("katomato.core.dispatcher.config.get_growth_phases", return_value={"veg": mock_phase}), \
            patch("katomato.core.dispatcher.config.switch_growth_phase") as switch_mock:
...
async def test_command_dispatcher_unknown_phase(command_queue, fake_arduino_protocol, fake_shutdown_handler, caplog):
    ...
    with patch("katomato.core.dispatcher.config.get_growth_phases", return_value={}):
